...
class Article(models.Model):
    ...
    views    = models.IntegerField(default=0)
    breviary = models.TextField(max_length=128, blank=True, verbose_name='')
    uuid = models.CharField(max_length=36)

    def save(self, *args, **kwargs):
        if self.breviary == '' or self.breviary == None:
            self.breviary = self.content[0:128]
        else:
...
class Mumbler(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.uuid == '' or self.uuid == None:
            self.uuid = str(uuid.uuid1())
        super(Mumbler, self).save(*args, **kwargs)
